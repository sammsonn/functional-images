...
instance Show RegionAST where
  show r = showIndented 0 r
    where
      showIndented indent (R (FromPoints points)) = replicate indent ' ' ++ "FromPoints " ++ show points
      showIndented indent (R (Rectangle w h)) = replicate indent ' ' ++ "Rectangle " ++ show w ++ " " ++ show h
      showIndented indent (R (Circle r)) = replicate indent ' ' ++ "Circle " ++ show r
      showIndented indent (R (Transform t r)) = replicate indent ' ' ++ show t ++ "\n" ++ showIndented (indent + 2) r
      showIndented indent (R (Complement r)) = replicate indent ' ' ++ "~" ++ "\n" ++ showIndented (indent + 2) r
      showIndented indent (R (Union r1 r2)) = replicate indent ' ' ++ "+" ++ "\n" ++ showIndented (indent + 2) r1 ++ "\n" ++ showIndented (indent + 2) r2
      showIndented indent (R (Intersection r1 r2)) = replicate indent ' ' ++ "*" ++ "\n" ++ showIndented (indent + 2) r1 ++ "\n" ++ showIndented (indent + 2) r2
...
instance Functor TransformationShape where
  ...
  fmap f (Combine xs) = Combine (map f xs)
...
instance Functor RegionShape where
  ...
  fmap f (FromPoints ps) = FromPoints ps
  fmap f (Rectangle w h) = Rectangle w h
  fmap f (Circle r) = Circle r
  fmap f (Complement a) = Complement (f a)
  fmap f (Union a1 a2) = Union (f a1) (f a2)
  fmap f (Intersection a1 a2) = Intersection (f a1) (f a2)
  fmap f (Transform t a) = Transform t (f a)
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea lenesa ajuta atunci cand vrem sa accesam doar o componenta
    a perechii, deoarece ne permite sa calculam doar componenta de care avem nevoie
    si sa o ignoram pe cealalta. Acest lucru poate economisi timp de calcul daca
    componenta ignorata este costisitoare de calculat.
