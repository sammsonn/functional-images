...
rectangle width height (x, y) = max (abs x / width) (abs y / height) <= 0.5
...
-}
circle :: Float -> Region
circle radius (x, y) = sqrt (x * x + y * y) <= radius
...
plot width height region = intercalate "\n" $ map plotRow [height, height - 1 .. -height]
  where
    -- \| genereaza o linie a reprezentarii, corespunzatoare unei anumite valori 'y'
    plotRow y = map (plotPoint y) [-width .. width]
    -- \| determina daca un punct dat '(x, y)' se afla in regiune sau nu si returneaza caracterul corespunzator
    plotPoint y x = if region (fromIntegral x, fromIntegral y) then '*' else '.'
...
promoteBinary f pointed1 pointed2 = f <$> pointed1 <*> pointed2
...
{-
    *** TODO ***
    ...
    Răspuns: evaluarea lenesa este benefica aici deoarece permite functiei sa
    genereze doar cat este necesar pentru a determina daca un anumit punct se
    afla in regiune. Daca un punct este gasit intr-unul dintre cercuri, functia
    poate returna True fara a genera restul cercurilor.
-}
circles :: Int -> Region
circles n
  | n <= 0 = const False
...
{-
    *** TODO ***
    ...
    Răspuns: daca un punct nu este in primul cerc, functia va verifica
    urmatorul cerc din reuniune, iar acest proces continua pana cand punctul
    este gasit intr-un cerc. Astfel, se poate poate ajunge la un ciclu infinit
    si la blocarea programului daca se incearca verificarea apartenentei unui
    punct care nu apartine regiunii.
-}
...
bfs start expand = breadthFirstSearch [(start, 0)] S.empty
  where
    -- \| este functia recursiva care implementeaza algoritmul BFS
    -- \| ea primeste o lista de noduri de vizitat si un set de noduri deja vizitate
    breadthFirstSearch [] _ = []
    breadthFirstSearch ((currentState, currentLevel) : remainingStates) visitedStates
      -- \| daca nodul curent a fost deja vizitat, trecem la urmatorul nod
      | currentState `S.member` visitedStates = breadthFirstSearch remainingStates visitedStates
      | otherwise =
          -- \| generam nodurile vecine ale nodului curent si filtram cele deja vizitate
          let expandedStates = expand currentState
              unvisitedExpandedStates = filter (`S.notMember` visitedStates) expandedStates
              -- \| adaugam nivelul curent + 1 la nodurile nou generate
              newStates = map (,currentLevel + 1) unvisitedExpandedStates
           in -- \| adaugam nodul curent la rezultat si continuam cautarea cu restul nodurilor si nodurile nou generate
              (currentState, currentLevel) : breadthFirstSearch (remainingStates ++ newStates) (S.insert currentState visitedStates)
...
regionAvoidingBfs start region = bfs start expandFunction
  where
    -- \| functie care, pentru un punct dat, returneaza vecinii acestuia care nu au fost inca vizitati
    expandFunction point = getUnvisitedNeighbors point region

-- | functie care returneaza vecinii unui punct dat care nu se afla intr-o regiune data
getUnvisitedNeighbors :: Point -> Region -> [Point]
getUnvisitedNeighbors (x, y) region = filter isNotInRegion neighborPoints
  where
    -- \| punctele adiacente punctului (x, y).
    neighborPoints = [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]
    -- \| functie care verifica daca un punct dat nu se afla in regiune
    isNotInRegion = not . region
